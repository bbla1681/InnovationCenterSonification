...
def df_to_midi(df, filename, note_column, velocity_column, time_column, beats, bpm, notes, vel_min, vel_max):
...
def api_to_midi( start, end, filename, note_column, velocity_column, time_column, beats, bpm, notes, vel_min, vel_max,):
    api_df = get_data(index=1, lim=100, start=start, end=end)
    
    df_to_midi(api_df,filename, note_column, velocity_column, time_column,beats, bpm , notes,vel_min, vel_max)
